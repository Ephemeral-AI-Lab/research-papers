...
An isolated campaign requires an exact marker-owned run directory, canonical gateway binary, and fixed architecture-specific Linux ELF daemon package. Startup creates a marker-owned `runtime/<run-id>` with mode-0600 token, config, owner-process metadata, and sanitized JSONL logs. Readiness requires both the gateway-owned PID file and an authenticated `list_sandboxes` response. Shutdown attempts the final sandbox sweep, process-group termination, labeled container/volume cleanup, log persistence, token deletion, and runtime removal independently; any failure blocks cleanup and preserves marker-owned evidence.
