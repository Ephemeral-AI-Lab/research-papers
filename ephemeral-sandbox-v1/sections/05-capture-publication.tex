...
has a narrower exception for an unsupported-special-file drop, so the baseline
...
public head at the audited Linux baseline.  It does not establish semantic
merge correctness, a cross-platform crash proof, or an unlimited transaction
size, and source inspection remains provisional until the paper snapshot is
frozen and fault evidence is archived.  Staging remnants, retained sessions,
lease release, cleanup failure, compaction, and reclamation are lifecycle and
recovery concerns, which Section~\ref{sec:lifecycle-recovery} treats
separately.
